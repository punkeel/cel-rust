...
use crate::vm::filter_tree::FilterNode;
...
impl Filter {
    ...
    pub fn from_expression(expression: Expression, schema: &Schema) -> Result<Self, String> {
        ...
        let (tree, var_names, var_indices) = if let Some(t) =
            filter_tree_compiler::compile_filter_tree_with_schema(
                &expression,
                &field_names,
                &bool_fields,
            )
            .ok()
            {
                ...
                (Some(t.root), t.var_names, indices)
            } else {
    ...
    pub fn eval_bool(&self, ctx: &EvalContext) -> bool {
        self.tree.as_ref().unwrap().eval_bool(&ctx.view())
    }
